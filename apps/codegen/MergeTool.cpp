...
void print_function(const vector<RewriteRule> &rules, const std::string &func_name, std::ostream &stream) {
    shared_ptr<Node> root = create_graph(rules, "expr");
    assert(root->children.size() == 1);
    root = root->children[0];

    stream << "Expr " << func_name << "(const Expr &expr, Simplify *simplifier) {\n";
...
int main(void) {
    ...
    std::ofstream codegen_file("gen/merge_tool_code.cpp", std::ios_base::out | std::ios_base::app );
    print_function(rules, "simplify_sub", codegen_file);
